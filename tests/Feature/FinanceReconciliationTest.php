...
it('reports every check and passes on a clean ledger', function () {
    ...
    expect(array_column($report['checks'], 'key'))->toBe([
        'customer_wallet_drift', 'game_wallet_drift', 'competition_wallet_drift', 'ledger_arithmetic', 'ledger_balance',
        'unclassified_entries', 'unmatched_deposits', 'deposits_without_ledger', 'stuck_withdrawals',
        'failed_withdrawals_not_reversed', 'stuck_escrow', 'aged_escrow', 'negative_balances', 'house_cut_rates',
        'mpesa_balance_freshness', 'cash_coverage',
    ]);
    expect($report['status'])->toBe('pass');
    expect($report['counts'])->toBe(['pass' => 16, 'warn' => 0, 'fail' => 0]);
...
it('rolls the checks up to an overall status', function () {
    ...
    expect($report['counts'])->toBe(['pass' => 15, 'warn' => 1, 'fail' => 0]);
